...
Festivized weapons are marked with a lightbulb badge when their attributes include defindex `2053`, mirroring the in-game festive indicator. The backend precomputes an `is_festivized` flag and emits a `data-festive="1"` attribute so the template can render the badge directly, while `static/retry.js`'s `addFestiveBadges()` appends the icon for dynamically loaded cards by reading this flag or falling back to the card's attribute list. All item badges sit in a compact row anchored at the card's top-right, and the template filters out any upstream festive badges to avoid duplicates.
