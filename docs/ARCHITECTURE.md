...
A progress toast appears as soon as the user submits a scan. It uses a rotating Font Awesome spinner to show batch progress. A shared `scanToast` controller drives this toast for both initial scans and retry flows, ensuring it hides when all scans settle and that the Completed bucket collapses only when every scan succeeds. The controller adds a `.show` class so CSS fade transitions remain intact and exposes a `setProgress` helper plus legacy `updateScanToast`/`hideScanToast` wrappers for older calls. During refresh operations the "Refresh Failed" button is temporarily disabled and shows a spinner. Its positioning leaves room for the floating controls and clamps width on small screens so long messages remain readable.
