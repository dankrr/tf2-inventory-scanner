...
- The item modal is handled by `static/modal.js` and provides a `.modal-close` button bound to `closeModal` for accessibility. Helpers in `static/retry.js` inject a backdrop and center the modal on desktop or dock it as a bottom sheet on mobile.
