...
- A floating refresh button (`#refresh-floating-btn`) scrolls to the top and mirrors `#refresh-failed-btn`; update if retry logic changes. The Steam ID textarea auto-focuses on load to speed up input.
